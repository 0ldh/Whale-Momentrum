...
//* 시간별 배경이미지 가져오는 함수
const times = () => {
  if (dateHours.getHours() >= 6 && dateHours.getHours() < 12) { // 브라우저에서 받은 시간이 6~12 사이라면
    time = 'Morning' // time 변수에 morning 저장
    return images.morning[Math.floor(Math.random() * images.morning.length)] // 배경이미지 객체의 배열moring중 랜덤한 이미지 이름 return
  } else if (dateHours.getHours() >= 12 && dateHours.getHours() < 15) {
    time = 'Noon'
    return images.noon[Math.floor(Math.random() * images.noon.length)]
  } else if (dateHours.getHours() >= 15 && dateHours.getHours() < 20) {
    time = 'Afternoon'
